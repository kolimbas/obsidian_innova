...
> Credencial **`hubspot-blincer-apptoken`** (App Token, id `A3JekIL652cjutl4`) enganchada a `Resolve segment`, `Update HubSpot Contact`, `Get bounces & unsubs (24h)` y `Add to do_not_email` (`authentication: appToken`). `Send via platform` queda **disabled** sin credencial (OQ-1 plataforma). ✅ **Private App token válido cargado (2026-06-10)** — las llamadas HubSpot quedan operativas. **Pendiente (ahora desbloqueado por el token):** resolver `REPLACE_DO_NOT_EMAIL_LIST_ID` (la lista `do_not_email` se busca con `GET /contacts/v1/lists`; si no existe hay que crearla en HubSpot).
